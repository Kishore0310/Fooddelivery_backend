import { readData, writeData, getNextId } from '../utils/dataStorage.js';

const addFoodSimple = async () => {
  try {
    const args = process.argv.slice(2);
    if (args.length < 3) {
      console.log('Usage: node scripts/addFoodSimple.js <name> <price> <restaurantId> [description] [veg]');
      console.log('Example: node scripts/addFoodSimple.js "Cheese Pizza" 299 1 "Delicious cheese pizza" true');
      process.exit(1);
    }

    const [name, price, restaurantId, description = '', veg = 'true'] = args;

    const menuItems = await readData('menuItems');
    
    const newItem = {
      id: await getNextId('menuItems'),
      name,
      price: Number(price),
      restaurantId: Number(restaurantId),
      description,
      veg: veg === 'true',
      image: 'https://images.unsplash.com/photo-1574071318508-1cdbab80d002?w=200&h=200&fit=crop'
    };

    menuItems.push(newItem);
    await writeData('menuItems', menuItems);

    console.log('✅ Food item added successfully:');
    console.log(`   Name: ${newItem.name}`);
    console.log(`   Price: ₹${newItem.price}`);
    console.log(`   Restaurant ID: ${newItem.restaurantId}`);
    console.log(`   ID: ${newItem.id}`);
    
  } catch (error) {
    console.error('❌ Error adding food item:', error.message);
  }
};

addFoodSimple();